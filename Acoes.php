...
if (isset($_POST['aluno-update'])) {
    $aluno_id = mysqli_real_escape_string($conexao, $_POST['id_aluno']); // Corrigido o nome da variável
    $nome = isset($_POST['nome_aluno']) ? mysqli_real_escape_string($conexao, trim($_POST['nome_aluno'])) : "";
    $email = isset($_POST['email_aluno']) ? mysqli_real_escape_string($conexao, trim($_POST['email_aluno'])) : "";
    $data_nascimento = isset($_POST['dt_nasc']) ? mysqli_real_escape_string($conexao, trim($_POST['dt_nasc'])) : "";
    $curso = isset($_POST['curso_aluno']) ? mysqli_real_escape_string($conexao, trim($_POST['curso_aluno'])) : "";

    $sql = "UPDATE alunos SET nome_aluno='$nome', email_aluno='$email', dt_nasc='$data_nascimento', curso_aluno='$curso' WHERE id_aluno='$aluno_id'"; // Corrigido o SQL

    if (mysqli_query($conexao, $sql)) {
        if (mysqli_affected_rows($conexao) > 0) {
            $_SESSION["mensagem"] = "Aluno atualizado com sucesso"; // Mensagem corrigida
            header("Location: index.php");
            exit;
        }
    }

    $_SESSION["mensagem"] = "O aluno não foi atualizado, por favor verifique"; // Mensagem corrigida
    header("Location: index.php");
    exit;
}
